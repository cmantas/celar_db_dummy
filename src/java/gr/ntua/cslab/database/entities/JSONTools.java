...
public class JSONTools {
    ...
    public static Application parseApplicationDescription(JSONObject topJson, boolean store) {
        try {
            ...
            if(store) app.store();
            System.out.println("loading configuration for app:" + app);
            ...
            for (int i = 0; i < modules.length(); i++) {
                ...
                for (int j = 0; j < components.length(); j++) {
                    ...
                    ProvidedResource pr=Tables.provResTable.getResouceByType(c.getString("provided_resource"));
                    c.put("PROVIDED_RESOURCE_id", pr.id);
   ...
   public static Metric parseMetric(JSONObject mj, boolean store, PrintWriter out){
    ...
    public static JSONObject parseJson(String jsonStr, PrintWriter output) {
        if(jsonStr==null){
            output.println("ERROR: no input for requred parameter");
            return null;
        }
        try {
            ...
            output.println("ERROR: Malformed JSON input: " + malformed.getMessage());
        }
        return null;
    }
